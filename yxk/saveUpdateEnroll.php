...
{ 
	if (count($_POST["course"]) != 3)
	{
		...
		echo "<script>window.location.href='updateEnroll.php';</script>"; 
	}
			
	$password = trim($_POST['password']);
	$studentID = $_POST['studentID'];
	
	$sql_update = "UPDATE km_tb SET password = '$password' WHERE id = '$studentID'"; 
	...
	$mysqli->close();
	echo "<script>alert('修改选课信息成功！');</script>";
